...
function InschrijfFormProduct() {
    ...
    async function handleAdjustSubmit(data, productName) {
        setIsSubmitting(true);
        try {
            await axios.put(`http://localhost:8081/products/${productName}`, data, {
                headers: {
                        ...
                        <div>
                            <form className=" form-container form-content border_top_bottom background"
                                ...
                                <div>
                                    ...
                                    <label>Type:</label>
                                    <select name="type"
                                            id="type" {...register('type', {required: 'type is verplicht'})}>
                                ...
                                <div>
                                    <label>Kleur:</label>
                                    <select name="color"
                                            id="color" {...register('color', {required: 'Kleur is verplicht'})}>
                                        ...
                                        <option value="zeer donker bruin (doorschijnend)">Zeer donker bruin
                                            (doorschijnend) ( groter dan 75 EBC)
                                        </option>
                                        <option value="zwart (niet doorschijnend)">Zwart (niet doorschijnend) ( groter
                                            dan 120 EBC)
                                        </option>
                                    </select>
                                </div>
                                ...
                                <button className="bttn" type="submit" disabled={isSubmitting} onClick={handleAdjustSubmit}>
                                    {isSubmitting ? 'Bezig met een product toevoegen momentje...' : 'aanpassen'}
                                </button>
                            </form>
                            <div>
                                <p>Vragen over de terminologie in het bierproces en omschrijving of wilt je meer
                                    informatie, ga dan naar: <NavLink to="/Productie_Informatie#algemene-informatie"
                                                                      onClick={scrollToAlgemene_info}>Hoe maak je
                                        bier</NavLink>.</p>
                            </div>
                        </div>
                        <Cubes
